...
import { ExtensionUtility } from "src/utils/ExtensionUtility";
...
function createAutoNote(
  ...
  targetNoteLineGuid: GUID,
  autoNoteLayerGuid: GUID
) {
  const { NoteRecord, guid, Fraction } = extensionUtility;

  // 同じロングの同じ位置に配置されているノートを探す
  const oNote = chart.timeline.notes.find(
    (note) =>
      note.type == "auto" &&
  ...
  // 既に配置されているノートにマージする
  if (oNote) {
    oNote.horizontalPosition.numerator = Math.min(
      oNote.horizontalPosition.numerator,
      horizontalPosition.numerator
    );

    oNote.horizontalSize =
      Math.max(
        oNote.horizontalPosition.numerator + oNote.horizontalSize,
        horizontalPosition.numerator + horizontalSize
      ) - oNote.horizontalPosition.numerator;

    return;
  }

  chart.timeline.addNote(
    NoteRecord.new(
      {
        guid: guid(),
        horizontalSize,
        ...
        measurePosition,
        type: "auto",
        lane: chart.timeline.lanes.find(
          (lane) => lane.templateName === "24-divisions"
        )!.guid,
...
function updateAutoNote(chart: Chart) {
  return;
  /*
...
const eventListener: IMusicGameSystemEventListener = {
  onSerialize(chart: Chart) {
    const { getHead, getTail } = CustomRendererUtility;

    for (const note of chart.timeline.notes) {
      const headhead = Array.from(getHead(note)).length;
      // 中間ノートか末尾ノート
      if (headhead) {
        const tail = Array.from(getTail(note)).length;
